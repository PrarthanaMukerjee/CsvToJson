...
var rlemitter = rl.createInterface(
  ...
  rlemitter.on('line',function(line) {
    ...
    if(head) {
      ...
      {
        if(key[i].indexOf("Educational level")>-1&&key[i].indexOf("Persons")>-1) //getting headers
        {
          array.push(i);// storing header specific values
        }
      }
    }
    else {
      ...
      if(data[5]=="All ages"&&data[4]=="Total")
      {
        for(var i=0;i<array.length;i++)
        {
          if(arr.length>i)
          {
            ...
            arr[i][k[1]] += Number(data[array[i]]); // adding the population
          }
          else {
            var obj = {};
            obj["educat"]= generateValue(key[array[i]]);
